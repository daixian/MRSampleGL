...
int ControllerFormGL::command(int id, int command, LPARAM msg)
{
    ...
    case IDC_BUTTON_VIEW_VRMODE:
        if (command == BN_CLICKED)
        {
            ...
            ::SetWindowLongPtr(mainWinHandle, GWL_STYLE, Style);
            ...
            Win::log(L"ControllerFormGL.command():显示器个数:%d 个", mInfo.size());
            if (mInfo.size() == 1)//如果只有一台显示器
            {
                RECT monitorRect = mInfo[0].rcMonitor;

                width = monitorRect.right - monitorRect.left;
                height = monitorRect.bottom - monitorRect.top;

                startx = monitorRect.left;
                starty = monitorRect.top;
            }
            else
            {
                for (int i = 0; i < mInfo.size(); i++)
                {
                    ...
                    //如果不是主屏，则显示该窗口。
                    if (mInfo[i].primary == FALSE)
                    {
                        RECT monitorRect = mInfo[i].rcMonitor;
            ...
            Win::log(L"ControllerFormGL.command():VR模式窗口:x=%d,y=%d, w=%d,h=%d", startx, starty, width, height);
            //把窗口全屏到副显示器上
...
static BOOL CALLBACK MonitorEnumProc(__in  HMONITOR hMonitor, __in  HDC hdcMonitor, __in  LPRECT lprcMonitor, __in  LPARAM dwData)
{
    ...
    AllMONITORINFO allInfo;
    allInfo.hMonitor = hMonitor;
    allInfo.rcMonitor = *lprcMonitor;

    //获取主屏的指针
    HMONITOR priMonitor = MonitorFromWindow(nullptr, MONITOR_DEFAULTTOPRIMARY);
    if (priMonitor == hMonitor)
        allInfo.primary = TRUE;
    else
        allInfo.primary = FALSE;

    infoArray.push_back(allInfo);
    return TRUE;
}
